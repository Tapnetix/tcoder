...
Use this template when dispatching a design-reviewer agent. The agent's static behavior (8-point design checklist, output format, severity mapping, review-summary format) is defined in the `tcoder:design-reviewer` agent definition. This template provides only the dynamic per-invocation context.

## Variables

- `{DESIGN_DOC_PATH}` — path to the design doc
- `{REPO_PATH}` — repository root path (for reading existing files to verify paths)
- `{DESIGN_REVIEWER_MODEL}` — model for the reviewer agent (from tcoder-settings)
