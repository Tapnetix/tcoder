...
E2E specs are owned per task (schema 2): a task with `e2e_scenarios` writes its own spec at `<e2e.spec_dir>/<task_id_lower><ext>`, runs the per-runner filtered command as part of TDD, and the orchestrator re-runs the union at every phase end. Scenario IDs are declared once in plan-level `e2e.scenarios` and referenced by tasks via `e2e_scenarios` (the validator derives spec paths from this mapping). Plans on the old schema (a single shared `e2e-red`/`e2e-green` pair) need to migrate — see validate-plan / validate-design for schema 2 enforcement.
